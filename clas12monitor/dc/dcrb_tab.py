...
class DCRB(QtGui.QTabWidget):
    def __init__(self, parent=None):
        super(QtGui.QTabWidget, self).__init__(parent)
        self.parent =  parent
        curdir = os.path.dirname(os.path.realpath(__file__))
        uic.loadUi(os.path.join(curdir,'ui','DCRB.ui'), self)
        self.init_buttons()
    ...
    def init_buttons(self):
        ...
        for sector_id,sector in enumerate(self.sectors):
            ...
            for sector_id,sector in enumerate(self.sectors):
                sector.clicked.connect(self.sendDCRBArray)
                superlayers = self.superlayers[sector_id]
                for superlayer_id,superlayer in enumerate(superlayers):
                    superlayer.clicked.connect(self.sendDCRBArray)
                    boards = self.boards[sector_id][superlayer_id]
                    for board_id,board in enumerate(boards):
                        board.clicked.connect(self.sendDCRBArray)

        self.currentChanged.connect(self.sendDCRBArray)
...
if __name__ == '__main__':
    ...
    class MainWindow(QtGui.QMainWindow):
        def __init__(self):
            ...
            self.dcrb_tab = DCRB()
            self.setCentralWidget(self.dcrb_tab)
            self.show()
